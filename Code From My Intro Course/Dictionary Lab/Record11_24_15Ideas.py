...
class Dictionary2:
    ...
    def lookup(self, key):
        precondition(True)
        c_node = self.d
        if self.d == [None, None]: return None #just too tired..
            
        while c_node.get_val() is not None and c_node.get_val()[0] != key:
            if key < c_node.get_val()[0]:
    ...
    #  d.put(k, v) returns the updated d, so d.put(k1, v1).put(k2, v2) is legal
    def put(self, key, value):
        precondition(type(key) == str)
        c_node = self.d
        while not c_node is None and c_node.get_val()[0] != key:
            if key < c_node.get_val()[0]:
                previous = [c_node, 'left']
                c_node = c_node.get_left()
            else:
                previous = [c_node, 'right']
                c_node = c_node.get_right()
            if c_node is None:
                if previous[1] == 'left': previous[0].left = [key, value]
                else: previous[0].right = [key, value]
            else: c_node.value = [key, value]
    ...
    def __eq__(self,otherDictionary):
        ...
        return self.d == otherDictionary.d
