...
def generate_no_loops2(states, colors):
    #precondition: states are single letters in a string with no spaces- '' is allowed
    #also:
    Preconditon(len(colors)> 0)
    #postcondition: prefix_function returns every possible coloring:
        #this includes every state with every color with each combination of other states and colors
    return prefix_function2("", states, colors) + "\n" #this is for consistency         
                                                   
def prefix_function2(prefix, states, colors):
    precondition( all([type(prefix)==str, type(states)==str, type(colors)==str]) and len(states) >= 0 and len(colors) >= 1)
    if len(states) ==0:
        #postcondition: prefix[:-1] has every possible coloring
        return prefix[:-1]
    else:
...
#====================My sneaky recursion method for n colors:============================
    #Note to a potential peer reviewer:
    #I have left out most comments and put them at the bottom of this file
    #So you can try to figure this out without help- should you accept
    #the challange!       
def generate_no_loops(states, colors):
    return prefix_function("", states, colors)                                                                                                                                                                         
def prefix_function(prefix, states, colors):
    return prefix[:-1] if len(states)==0 else prefix_per_color('', colors, states, colors, prefix) #we trim the '/n'in the base case
def prefix_per_color(ret, remaining_colors, states, colors, prefix):
    if remaining_colors == '': return ret
    else:
...
#========================================================================================
        ...
        #third way:
#========================================================================
#A poor way of doing it with loops yet still using string representation:
#========================================================================   
def triple(one,coloring_ommiting_one,colors):
    #Insert precondition: not sure how but make sure the coloring omitting one is in the right format
    #and colors are lower case different letters consecutively in a string corresponding to the coloring given
    colorings_lst = coloring_ommiting_one.split('\n')[:-1] # we remove the last empty element and make a list of colorings
    ret = []
#for each color we add the one state plus that color to the list of colorings
#and combine all these in one new list called ret:
    while len(colors) > 0:
        for i in colorings_lst:
            ret += [one + colors[0] + ' ' + i]
        colors = colors[1:]
#we need to convert this list-ret,  to a string with '\n' added after each element:
#this wouldn't be necessary if we were representing stuff in a non-stupid manner   
    s_ret = ''
    for a in ret:
        s_ret += a + '\n'
    #postcondition: s_ret is a string with each coloring having len(colors) new colorings
    # which essentially makes len(colors)**len(colorings_lst) new colorings in a string            
    return s_ret
def generate(states, colors):
    precondition(len(states) >= 1 and len(colors) >= 1)
    #the base case, return combination of the state with each color
    if len(states) == 1:
        ret = ''
        for i in colors:
            ret += states + i + '\n'
        #postcondition: every combination of coloring, with each state 
        #varying in color- in total, len(colors)**len(states) colorings are returned     
        return ret
    #recursive case: return the first state added to the list of colorongs without
    #the first state using the same colors obviously
    else:
        #same postcondition as above
        n_ret = triple(states[0], generate(states[1:], colors), colors)
        return n_ret
        
#============================================================================
# End of the poor way of doing it with loops and using string represnatation
#===========================================================================
...
def collect_legal_colorings(states, colors, borders):
    ...
    precondition(colors == 'rgb' and len(states) >= 1)# amapt wihout states is nonsensical
        #also you need to give real borders corresponding to the states and colors
        #however, we can use sneaky recursion and remove the 3 color restraint
        #to do so just change generate_no_loops2 to generate_no_loops
    ...
    if MODE=='mine':
        if borders == '':
            #postcondition: all colorings that correspond to the states given are returned
            answer = generate_no_loops(states, colors) #you could use generate_no_loops2
            
        else:
            #postcondition: only colorings that do not have any two adjacent states with the same color
            #are returned in a string seperated by "\n"s
...
#####################################################################
            #Comments for sneaky recusive generate.
#####################################################################
#The prefix_function function gives you a prefix 
#(which is just a portion of one coloring like "Cr " -notice the space)
...
#Test_prefix_Ag Bg
#            
#The prefix_per color function varies the coloring of the
#"leading" state and returns all possible colorings with that change
#and a prefix before it
#Thus it creates a prefix_function call per each coloring of the
#remaining_colors with the first state
    #Ex:
#>>> print prefix_per_color('', 'gb', 'AB', 'rgb', 'Test_prefix_')
#Test_prefix_Ag Br
#Test_prefix_Ag Bg
#Test_prefix_Ag Bb
#Test_prefix_Ab Br
...
#Test_prefix_Ab Bb
    ...
    #each of the remaining colors 'gb' and that new prefixes "Test_prefix_Ag "
    #(notice the space at the end) and "Test_prefix_Ab " are added to 
    #each coloring of state "B" with colors "rgb"
#