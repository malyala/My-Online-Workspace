...
#This is a helper function that I use in the algorithm below    
    def intersect(val, center_a, center_b, radius, vmin, vmax):
            if abs(val - center_a)<= radius:
                a = center_b + (radius**2 - (val - center_b)**2)**0.5
                b = center_b - (radius**2 - (val - center_b)**2)**0.5
                if a >= vmin and a <= vmax:
                    return True
                if b != a and b >= vmin and b <= vmax:
                    return True
...
#Part 2: Check if there are any intersections- if any return true
    ...
    #This is commented because it fails..- check the function above in detail to fix it
                        
        #return any(map(intersect, [xmin, xmax, ymin, ymax], [center_x, center_x, center_y, center_y], \
        #[center_y, center_y, center_x, center_x], [radius for i in range(4)], [ymin, ymin, xmin, xmin], \
        #[ymax, ymax, xmax, xmax]))
