...
class App:
    ...
    def __init__(self, master):
        ...
        self.labela = Label(self.framea, text="size of plate (cm):", bg="light blue")
        ...
        self.plate = create_plate(int(self.entrya.get()), float(self.entry1.get()))
        self.draw_plate(self.plate)
    ...
    def nextframe(self):
        simulate_plate(self.plate,int(self.entry2.get()),int(self.entry3.get()),
                       int(self.entry4.get()),int(self.entry8.get()),
                       float(self.entry5.get()),float(self.entry6.get()),float(self.entry7.get()))
        self.update_plate()
    
    def donsteps(self):
        simulate_plate_N_steps(self.plate,int(self.entry2.get()),int(self.entry3.get()),
                               int(self.entry4.get()),int(self.entry8.get()),
                               float(self.entry5.get()),float(self.entry6.get()),float(self.entry7.get()),
                               int(self.entry9.get()))
        self.update_plate()
    ...
    def apply_iron(self, event):
        self.plate[int(event.x/10)][int(event.y/10)] = int(self.entry4.get())
        self.update_plate()
    
    def draw_plate(self, plate):
        self.photo = PhotoImage(width=len(plate), height=len(plate[0]))
        for x in range(len(plate)):
            self.photo.put(tuple(map(self.get_color,plate[x])), to=(x,0))
        self.photo = self.photo.zoom(500/len(plate),500/len(plate[0]))
